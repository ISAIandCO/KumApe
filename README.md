...
> [!IMPORTANT]
> Версия `0.2.0` — технический preview для KUMA 4.6. Используемые REST-маршруты сверены с официальной схемой KUMA Public API 3.0; DOM и локальные mappings процессов всё ещё нужно проверить на реальной инсталляции.
...
- отдельная вкладка с таблицей результатов, SQL и полным JSON событий;
- «Полезные фильтры»: входы, процессы, PowerShell, службы, сеть/DNS, файлы, хеши и правила корреляции;
- поиск одного слоя родительских/дочерних процессов по настраиваемым PID-полям;
...
Для поиска связанных процессов профиль может дополнительно содержать `processGraph` с массивами полей `host`, `pid`, `parentPid`, `processGuid`, `parentGuid`, `image`, `commandLine`, `user` и `eventId`. Встроенные mappings для 4688, Sysmon 1 и auditd EXECVE являются стартовыми: перед использованием их нужно сверить с нормализаторами конкретной KUMA.
...
- интерактивный process graph и автоматическое многошаговое расширение цепочки;
