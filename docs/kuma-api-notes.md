...
**Права:** ключу нужны GET `/api/v3/events/clusters` и POST `/api/v3/events`. Поиск использует POST с SQL в теле, хотя не изменяет события. При GET-only правах ожидаем 403 `access denied`. Если POST разрешён, проверьте доступ пользователя к кластеру и тенанту. KumApe показывает подсказку для этого ответа и не пытается обходить отказ через другую сессию.
...
Получение события, related search через Public API и формирование SQL подтверждены на целевой KUMA. Локальный workspace и типизированные полезные фильтры включены с версии 0.2.0, интерактивный process graph — с 0.2.1. Фактические поля PID/GUID/Parent PID задаются отдельно для каждого Event ID. Для нативного поиска используется проверенный URI `/threat-hunting#/threat-hunting?search=...`: параметр `search` содержит дважды URL-закодированный JSON с `sql` и относительным `period`. DOM-подстановка запроса больше не используется. Изменяющие операции с Active Lists остаются вне read-only политики KumApe.
