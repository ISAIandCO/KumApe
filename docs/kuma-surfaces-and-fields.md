...
## Отдельные поля графа процессов

Граф не использует JSON-профили related search. Каждая запись формы сопоставляет точное значение Event ID с полями нормализованного события: `host`, `pid`, `parentPid`, необязательными GUID, image, command line, user и ID события KUMA. В запрос попадают только события настроенных типов на том же узле. Связь по GUID имеет приоритет; при связи по PID выбирается ближайший предшествующий запуск на том же узле, чтобы ограничить ошибочные связи при повторном использовании PID.
