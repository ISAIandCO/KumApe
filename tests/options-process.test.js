...
function options(mapping) {
  ...
  const element = id => {
    if (!elements.has(id)) elements.set(id, { style: {}, handlers: {}, addEventListener(name, fn) { this.handlers[name] = fn; } });
    return elements.get(id);
  };
  const writes = [];
  const context = vm.createContext({ URL, TextEncoder,
    document: { querySelector: element, querySelectorAll: () => [{ querySelectorAll: () => Object.entries(mapping).map(([key, value]) => ({ dataset: { key }, value: String(value) })) }] },
