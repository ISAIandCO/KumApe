...
export function buildProcessGraphView(graph, sourceNodeId) {
  ...
  const nodes = graph.nodes.map(n => {
    ...
    const event = { kumaEvent: n.event, 'object.process.name':label, 'object.process.path':n.image, 'object.process.id':n.pid,
      'subject.account.name':n.user, 'event_src.host':n.host, msgid:n.mappingName };
    ...
    return { id:n.id, parentId:parents.get(n.id), depth:0, time:n.timestamp, event, connectionCount, radius:Math.min(34,10+Math.sqrt(connectionCount)*6), selected:n.id===sourceNodeId, label,
